...
public:
    ...
    void serializeBin(const string& filename) {
        ...
        if (file.is_open()) {
            ...
            // Сначала подсчитаем общее количество элементов
            for (int i = 0; i < size; i++) {
                NodeHT* current = table[i];
                while (current != nullptr) {
                    totalElements++;
            ...
            file.write(reinterpret_cast<char*>(&totalElements), sizeof(totalElements));
            
            // Затем записываем все элементы
            for (int i = 0; i < size; i++) {
                NodeHT* current = table[i];
                while (current != nullptr) {
    ...
    void deserializeBin(const string& filename) {
        ...
        if (file.is_open()) {
            ...
            // читаем размер таблицы и создаем новую
            int newSize;
            ...
            int totalElements;
            file.read(reinterpret_cast<char*>(&totalElements), sizeof(totalElements));
            
            // читаем и вставляем все элементы
            for (int i = 0; i < totalElements; i++) {
                ...
                file.read(valueBuffer, valueLen);
                valueBuffer[valueLen] = '\0';
                
                insert(string(keyBuffer), string(valueBuffer));
